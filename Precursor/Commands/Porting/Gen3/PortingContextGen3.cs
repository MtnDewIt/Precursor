...
namespace Precursor.Commands.Porting.Gen3
{
    public partial class PortingContextGen3 
    {
        ...
        public PortingContextGen3(GameCacheHaloOnlineBase cacheContext, GameCache blamCache, Stream cacheStream)
        {
        ...
        public void PortTag(string portingOptions, string tag)
        {
            ...
            using (var blamCacheStream = PortTagContext.BlamCache is GameCacheModPackage ? ((GameCacheModPackage)PortTagContext.BlamCache).OpenCacheRead(CacheStream) : PortTagContext.BlamCache.OpenCacheRead())
            {
                ...
                {
                    if (blamTag == null)
                        new PrecursorError($"The specified tag does not exist in the current tag cache: {tag}");
