...
namespace Precursor.Tests.Cache.BuildInfo.Gen4
{
    public class Halo4E3Info : BuildTableEntry
    {
        public override CacheBuild Build => CacheBuild.Halo4E3;
        public override CacheVersion Version => CacheVersion.Halo4E3;
        ...
        private FileValidationResult ValidateFileAsync(string filePath)
        {
            ...
            using (var reader = new EndianReader(stream))
            {
                if (!SharedFiles.Contains(fileInfo.Name))
                {
                    ...
                    {
                        return new FileValidationResult(false, $"Invalid Build String: {fileInfo.Name} - {mapFile.Header.GetBuildNumber()}\n" + $"\nValid Build Strings:\n" + $"{string.Join("\n", BuildStrings)}\n");
                    }
                }
